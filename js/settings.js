...
MyApp.controller('CardCtrl', function($scope,myutils,$http,$mdDialog) {
	...
	$scope.activeGroup = active;
	$scope.groups = nameGroups;
	$scope.groupsLen = Object.keys($scope.groups).length;
	$scope.openGroup = function(name) {
		localStorage.setItem('activeGroup', name.nameOfGroup);
		window.location = "tt.html";
    };
	$scope.deleteGroup = function(name) {
		localStorage.removeItem('timeTable'+ name.nameOfGroup);
		var retrievedObject = localStorage.getItem('groups');
		nameGroups = JSON.parse(retrievedObject);
		for (var key in nameGroups) {
			if(name.nameOfGroup == nameGroups[key].nameOfGroup) {
				delete nameGroups[key];
			}
		}
								...
								if ('a' in itemCell.p) {
									if ('content' in itemCell.p.a.font) {
										if (itemCell.p.a.font.content.indexOf(name) != -1) {
											var li = listOfGroup[1] = new Object();
